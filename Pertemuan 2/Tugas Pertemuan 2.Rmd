...
# Import Data
```{r}
data <- read_excel("C:\\Dokumen\\Kuliah IPB\\Semester 5\\Metode Peramalan Deret Waktu\\Praktikum\\File R\\mpdw\\Pertemuan 2\\Data Suhu Rata-Rata Harian Malaysia.xlsx", sheet = "500 Periode")
data1 <- data[301:400,]
...
# ubah ke time series
train_ma.ts <- ts(training_ma$Rata_Suhu)
test_ma.ts  <- ts(testing_ma$Rata_Suhu)
```
...
library(ggplot2)
ggplot() + 
  geom_line(data = training_ma, aes(x = Tanggal, y = Rata_Suhu, col = "Data Latih")) +
  geom_line(data = testing_ma, aes(x = Tanggal, y = Rata_Suhu, col = "Data Uji")) +
  labs(x = "Periode Waktu", y = "Rata-Rata Suhu", color = "Legend") +
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Rata-Rata Suhu", main= "SMA N=4 Data Rata-Rata Suhu")
points(data1.ts)
...
```{r}
ts.plot(data1.ts, xlab="Time Period ", ylab="Rata-Rata Suhu", main= "DMA N=4 Data Sales")
points(data1.ts)
...
Perhitungan akurasi menggunakan data latih menghasilkan nilai MAPE yang lebih dari 10% sehingga nilai akurasi ini dapat dikategorikan sebagai baik.
...
# ubah ke time series
train.ts <- ts(training$Rata_Suhu)
test.ts  <- ts(testing$Rata_Suhu)
```
...
library(ggplot2)
ggplot() + 
  geom_line(data = training, aes(x = Tanggal, y = Rata_Suhu, col = "Data Latih")) +
  geom_line(data = testing, aes(x = Tanggal, y = Rata_Suhu, col = "Data Uji")) +
  labs(x = "Periode Waktu", y = "Rata-Rata Suhu", color = "Legend") +
  scale_colour_manual(name="Keterangan:", breaks = c("Data Latih", "Data Uji"),
                      values = c("blue", "red")) + 
  theme_bw() + theme(legend.position = "bottom",
...
autoplot(ses.1) +
  ...
  ylab("Rata-Rata Suhu") + xlab("Periode")
```
...
resid1<-training$Rata_Suhu-ramalan1$fitted
head(resid1)
...
resid2<-training$Rata_Suhu-ramalan2$fitted
head(resid2)
...
e1   <- as.numeric(ramalan1$mean)[1:n_test] - as.numeric(testing$Rata_Suhu)
e2   <- as.numeric(ramalan2$mean)[1:n_test] - as.numeric(testing$Rata_Suhu)
eopt <- as.numeric(ramalanopt$mean)[1:n_test] - as.numeric(testing$Rata_Suhu)
...
```{r}
accuracy(ramalan1, testing$Rata_Suhu)
accuracy(ramalan2, testing$Rata_Suhu)
accuracy(ramalanopt, testing$Rata_Suhu)
```
...
selisihdes1<-ramalandes1$mean-testing$Rata_Suhu
selisihdes1

SSEtestingdes1<-sum(selisihdes1^2)
MSEtestingdes1<-SSEtestingdes1/length(testing$Rata_Suhu)
MAPEtestingdes1<-sum(abs(selisihdes1/testing$Rata_Suhu)*100)/length(testing$Rata_Suhu)

selisihdes2<-ramalandes2$mean-testing$Rata_Suhu
selisihdes2

SSEtestingdes2<-sum(selisihdes2^2)
MSEtestingdes2<-SSEtestingdes2/length(testing$Rata_Suhu)
MAPEtestingdes2<-sum(abs(selisihdes2/testing$Rata_Suhu)*100)/length(testing$Rata_Suhu)

selisihdesopt<-ramalandesopt$mean-testing$Rata_Suhu
selisihdesopt

SSEtestingdesopt<-sum(selisihdesopt^2)
MSEtestingdesopt<-SSEtestingdesopt/length(testing$Rata_Suhu)
MAPEtestingdesopt<-sum(abs(selisihdesopt/testing$Rata_Suhu)*100)/length(testing$Rata_Suhu)
...
akurasi1.train = data.frame(Model_Winter = c("Winter Aditif","Winter Aditif Optimal"),
...
akurasi2.train = data.frame(Model_Winter = c("Winter Multiplikatif","Winter Multiplikatif Optimal"),
...
# Kesimpulan
Berdasarkan hasil peramalan pada data uji, metode Single Exponential Smoothing (SES) dengan parameter optimum merupakan metode yang paling akurat untuk meramalkan data berdasarkan nilai MSE dan MAPE. Nilai MSE dan MAPE pada metode Single Exponential Smoothing (SES) dengan parameter optimum secara berturut-turut adalah sebesar 0.1793621 dan 1.301067%. Hal tersebut menunjukkan bahwa SES optimum merupakan metode peramalan terbaik untuk data rata-rata suhu yang digunakan.